...
def save_gt_img(parts, camera_kwargs, gt_transformation, render_kwargs, file, save_dir):
...
def sample_data_files(directory, num_samples=1097):
...
def main(cfg):
    save_dir = cfg.inference_dir
    os.makedirs(f"./render_results/{save_dir}", exist_ok=True)
    
    data_dir = os.path.join(cfg.experiment_output_path, "inference", cfg.inference_dir)
    ...
    for file in sampled_files:
        ...
        save_gt_img(parts, camera_kwargs, gt_transformation, render_kwargs, file, save_dir=save_dir)

        imgs = []
        for i in range(1, transformation.shape[0]):
            img = render_parts(parts, camera_kwargs, transformation[i], **render_kwargs)
            imgs.append(img)
        visutil.imgarray2video(targetPath=f"./render_results/{save_dir}/{file}_{acc}.mp4", img_list=imgs, duration=6, extend_endframes=50)
        print(f"Saved {file} to render_results/{save_dir}/{file}_{acc}.mp4")
